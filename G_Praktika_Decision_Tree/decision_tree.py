...
def decision_tree(X, Y, scale, level=0):
    ...
    for i in range(n):
        ...
        if scale[i] == CATEGORICAL:   # категориальный признак
            ...
            info_s = 0
            for a in range(len(np.unique(X[:, colomn]))):
                T = []
                for_info = []
                for j, index in zip(X[:, colomn], range(len(X[:, colomn]))):
                    if j == np.unique(X[:, colomn])[a]:
                        for_info.append(Y[index])
                        T=((np.sum((X[:, colomn]) == np.unique(X[:, colomn])[a])) / len(Y))
                info_s += T * Info(for_info)
            gain.append((Info(Y) - info_s))
            #info_s=Entropy_gain(Y, X[:,i])
            #gain.append(info_s)
