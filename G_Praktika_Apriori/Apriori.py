...
def support(X, candidate):
    sum=0
    for i in X:
        if i[candidate[0]]==1 and i[candidate[1]]==1:
            sum+=1
    return sum/len(X)
...
def apriori(X, minimal_support):
    ...
    v_hor_zeros_v1 = np.zeros((X.shape[1],))
    support_ = []  # поддержка каждого 1-элементного набора
    for i in range(X.shape[1]):
        sum(X[:, i])
        support_.append(sum(X[:, i]) / X.shape[0])
        v_hor_zeros_v1[i] = support_[i]
    support_ = v_hor_zeros_v1
     # поддержка каждого 1-элементного набора
    ...
    for k in range(2, n+1):
        ...
        for candidate in multi_element_candidates:
            if support(X, candidate) > minimal_support:
                ...
                accepted_candidates = np.append(multi_element_candidates[candidate],accepted_candidates).reshape(-1 , len(candidate))
